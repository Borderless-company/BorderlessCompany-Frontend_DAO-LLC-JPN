...
export const RenderCell = ({ item, columnKey }: Props) => {
  ...
  const cellValue = item[columnKey as keyof UtilityToken];
  switch (columnKey) {
    case "name":
    ...
    case "actions":
      return (
        <div className="relative flex items-center gap-2">
          <Link
            href={`/dao/${daoId}/utility-token/${item.tokenAddress}`}
            className="text-sm"
          >
            詳細
          </Link>
          <Link
            href={`/dao/${daoId}/utility-token/${item.tokenAddress}/mint`}
            className="text-sm"
          >
            発行
          </Link>
...
export default ListUtilityTokens;
